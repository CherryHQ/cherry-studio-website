...
const CssPage: React.FC = () => {
  ...
  // 修改加载数据函数
  const fetchCssData = useCallback(async (
    pageNum: number, 
    ...
    try {
      ...
      console.log(`加载数据${(tag || author) ? '(批量模式)' : ''}：第${pageNum}页，每页${limit}条${author ? `，筛选作者: ${author}` : ''}${search ? `，搜索: ${search}` : ''}${tag ? `，标签: ${tag}` : ''}`);
  ...
  }, [hasMore, loadingMore, fetchCssData, authorFilter, searchTerm, tagFilter, allLoadedItems, displayPage, ITEMS_PER_PAGE_DISPLAY]);
      ...
      <div className="css-submit-modal-overlay">
        <div className="css-submit-modal">
          <div className="css-submit-modal-header">
            <h2>主题投稿</h2>
            <button className="css-submit-modal-close" onClick={handleClose}>×</button>
          </div>
          ...
          <div className="css-submit-modal-content">
            {submitSuccess ? (
              <div className="css-submit-success">
                ...
                <button className="css-btn primary" onClick={handleClose}>关闭</button>
              </div>
            ) : (
                  ...
                  <div className="css-submit-form-group">
                    ...
                    <div className="css-image-upload">
                      {lightImage ? (
                        ...
                        <div className="css-image-dropzone" onClick={() => document.getElementById('light_pic')?.click()}>
                          <i className="css-icon-upload"></i>
                          <span>点击上传图片</span>
                          <div className="css-image-hint">支持 JPG、PNG、GIF、WEBP 格式，最大 2MB，图片比例建议 16:9，不符合将被裁切。</div>
                        </div>
                      )}
                      <input 
                        type="file" 
                  ...
                  <div className="css-submit-form-group">
                    ...
                    <div className="css-image-upload">
                      {darkImage ? (
                        ...
                        <div className="css-image-dropzone" onClick={() => document.getElementById('dark_pic')?.click()}>
                          <i className="css-icon-upload"></i>
                          <span>点击上传图片</span>
                          <div className="css-image-hint">支持 JPG、PNG、GIF、WEBP 格式，最大 2MB，图片比例建议 16:9，不符合将被裁切。</div>
                        </div>
          ...
          <div className="css-container">
              ...
              <div className="css-buttons-container">
                <div className="css-search-help-icon" 
                  ...
                  <div className={`css-search-tooltip ${helpTooltipVisible ? 'visible' : ''}`}>
                    <p>搜索提示:</p>
                    <ul>
                      <li>使用 <code>#关键词</code> 搜索包含该标签的内容</li>
                      <li>使用 <code>@作者名</code> 搜索特定作者</li>
                      <li>支持组合搜索，# @ 搜索内容用空格隔开</li>
                    </ul>
                  </div>
                </div>
                <button className="css-submit-button" onClick={() => setSubmitModalOpen(true)}>
                  投稿
                </button>
              </div>
            </div>
            ...
            {(authorFilter || tagFilter || (searchTerm && parseSearchInput(searchTerm).plainText)) && (
              <div className="css-filter-info">
                ...
                <button 
                  className="css-btn secondary small clear-filter-btn" 
                  onClick={clearAllFilters}
                >
                  清除全部筛选
                </button>
              ...
              <>
                    ...
                    <div className="css-card" key={item.id}>
                      ...
                      <div className="css-card-content">
                        ...
                        <div className="css-card-title-container">
                          {Array.isArray(item.title) ? (
                            ...
                            <span 
                              className={`css-card-title-tag ${tagFilter === (Array.isArray(item.title) ? item.title[0] : item.title) ? 'active' : ''}`}
                      ...
                      <div className="css-card-footer">
                        ...
                        <div className="css-card-info">
                          <span className="css-card-version-label">版本: <span className="css-card-version-value">v{formatVersion(item.version)}</span></span>
                          <span className="css-card-date-label">发布: <span className="css-card-date-value">{formatDate(item.date_created)}</span></span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                {cssItems.length === 0 && !loading && (
                  <div className="css-no-results">
                    ...
                    <button 
                      className="css-btn secondary clear-filter-btn" 
                      onClick={clearAllFilters}
                    >
                      清除筛选，查看所有素材
                    </button>
                ...
                <div className="css-load-more">
                  ...
                  {hasMore && !loadingMore && cssItems.length > 0 && (
                    <button 
                      className="css-btn primary load-more-btn" 
