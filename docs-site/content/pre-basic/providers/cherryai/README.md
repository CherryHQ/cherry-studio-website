...
**CherryAI** 是 Cherry Studio 官方内置的免费模型服务。无需注册第三方平台或填写 API Key，打开即可使用，适合首次体验对话和基础文本任务。

### 在哪里使用

打开 `设置 → 模型服务`，在内置列表中找到 **CherryAI**。该服务随软件预置，通常无需额外配置。

### 当前提供的模型

根据当前 CherryAI 服务端配置，CherryAI 仅提供 **Qwen3-8B**。

{% hint style="warning" %}
模型可用性、免费额度和限流可能随服务端配置调整。使用时请以 Cherry Studio 中实际显示并可调用的模型为准；本页不代表长期可用承诺。
{% endhint %}

若需要其他模型或更稳定的服务，可选择 [CherryIN](../cherryin-1.md) 或其他 [服务商](../README.md)。
