...
{% hint style="warning" %}
需要保留 V1 数据时，正确路径是：**V1.9.13 → V2（直接完成数据迁移）**，无需先安装特定的 V2 中转版本。
{% endhint %}
