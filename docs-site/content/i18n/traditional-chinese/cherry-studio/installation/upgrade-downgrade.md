...
{% hint style="warning" %}
需要保留 V1 資資時，正確路徑是：**V1.9.13 → V2.0.2（直接完成資資遷移）**。不再需要先安裝 V2.0.0。
{% endhint %}
