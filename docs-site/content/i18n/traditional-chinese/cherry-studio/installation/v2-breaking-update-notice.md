...
需要保留 V1 資資時，按 **V1.9.13 → V2.0.2（直接完成資資遷移）** 操作，不再需要 V2.0.0 中轉。

| 目前情況 | 應該怎麼做 |
| --------------- | --------------------------------- |
| 仍在 V1，需要保留資資 | 將 V1 更新到 1.9.13，完整備份後直接安裝 V2.0.2。 |
| 已經在使用 V2 | 正常升級 V2.0.2，繼續使用目前 V2 資資。 |
| 之前遷移 V1 失敗或遺漏資資 | 先完整備份目前 V2，再考慮使用【重新遷移】。 |
| 不需要 V1 資資 | 可以選擇【忽略並使用預設值】，但 V1 資資不會遷入。 |

{% hint style="danger" %}
正常升級 V2.0.2 不需要點擊【重新遷移】。該操作會永久刪除目前 V2 資資，再從原始 V1 資資重新匯入；除非此前 V1 遷移失敗或遺漏資資，否則一定不要點擊。
{% endhint %}
...
V2.0.2 在【設定】→【資資】中增加【重新遷移】。它只用於修復此前 V1 遷移失敗或遺漏資資的情況。
