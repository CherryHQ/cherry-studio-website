...
為了讓每一位開發者和使用者都能輕鬆體驗前沿大模型的能力，**智譜免費為 Cherry Studio 的使用者開放了 GLM-4.5-Air 模型**。作為專為智能體（Agent）應用打造的高效基礎模型，GLM-4.5-Air 在效能與成本之間實現了出色平衡，是構建智能應用的理想選擇。
...
透過精簡設計，GLM-4.5-Air 實現了更高的推理效率，適合在資源受限環境下部署，同時仍能勝任複雜任務處理。
...
3. **長上下文支援**：上下文長度擴展至 **128K tokens**，可處理長文件、複雜對話或大型程式碼專案；
4. **強化學習增強**：透過 RL 優化模型在推理規劃、工具呼叫等方面的決策能力。
...
✅ **工具呼叫支援**：可透過標準化介面呼叫外部工具，實現任務自動化\
✅ **網頁瀏覽與資訊擷取**：可配合瀏覽器外掛完成動態內容理解與互動\
...
GLM-4.5-Air 支援 **混合推理模式**，使用者可透過 `thinking.type` 參數控制是否啟用深度思考：

* `enabled`：啟用思考，適合需要分步推理或規劃的複雜任務
* `disabled`：停用思考，用於簡單查詢或即時回應
* 預設設定為 **動態思考模式**，模型自動判斷是否需要深入分析

| 任務類型 | 範例 |
| ------------------ | ----------------------------------------------- |
| **簡單任務**（建議關閉思考） | <p>- 查詢「智譜 AI 的成立時間」<br>- 翻譯「I love you」為中文</p> |
| **中等任務**（建議啟用思考） | <p>- 比較飛機與高鐵從北京到上海的優劣<br>- 解釋木星為何有較多衛星</p> |
| **複雜任務**（強烈建議啟用思考） | <p>- 說明 MoE 模型中專家如何協作<br>- 基於市場資訊分析是否應買入 ETF</p> |

***

**🌟 高效低成本，部署更輕鬆**

GLM-4.5-Air 在效能與成本之間實現了優秀平衡，特別適合實際業務部署：

* ⚡ **生成速度超 100 tokens/秒**，回應迅速，支援低延遲互動
* 💰 **API 成本極低**：輸入僅 **0.8 元/百萬 tokens**，輸出 **2 元/百萬 tokens**
* 🖥️ 激活參數少，算力需求低，易於在本地或雲端高併發執行
