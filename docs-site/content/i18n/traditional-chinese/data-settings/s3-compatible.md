...
Cherry Studio 資資備份支援透過 S3 相容儲存（物件儲存）的方式進行備份。常見的 S3 相容儲存服務有：AWS S3、Cloudflare R2、阿裡雲 OSS、騰訊雲 COS 以及 MinIO 等。

基於 S3 相容儲存可以透過 `A電腦`  → (備份) →  `S3儲存`  → (恢復) →  `B電腦` 的方式來實現多端資資同步。

### 配置 S3 相容儲存

1.  建立物件儲存桶（Bucket），並記錄下儲存桶名稱。**強烈建議將儲存桶設定為私有讀寫以避免備份資資洩露！！**
2.  參考文件，前往雲服務控制台取得 S3 相容儲存的 `Access Key ID`、`Secret Access Key`、`Endpoint`、`Bucket`、`Region` 等資訊。
    ...
    - **相關文件**
        ...
        - 阿裡雲 OSS：[取得 Access Key ID 和 Access Key Secret](https://help.aliyun.com/zh/oss/developer-reference/use-amazon-s3-sdks-to-access-oss#306596478ed3r)
