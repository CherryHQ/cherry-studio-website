...
本頁展示 [Arena AI](https://arena.ai/leaderboard/agent) 排行榜前 30 名的每日快照，方便快速了解近期模型表現。完整排行榜、篩選選項和最新變化請到 Arena AI 官網查看。

本排行榜評測模型在多輪工具呼叫 / 智能體任務上的綜合表現，涵蓋淨改進率、確認成功率、讚踩比、可控性、Bash 恢復率、工具幻覺率等維度。

> **資資更新時間**: 2026-09-19 12:14:58 UTC / 2026-09-19 20:14:58 CST (北京時間)

{% hint style="info" %}
排行榜反映特定評測和用戶投票偏好，不等同於模型在你的任務中一定更好。選擇模型時還要考慮價格、速度、上下文、工具呼叫、隱私和地區可用性。
{% endhint %}
...
* **排名 / 排名區間**：Arena AI 根據智能體任務對戰投票估算的相對名次；排名區間表示在置信範圍內的名次波動。
* **淨改進率**：相對於基線在多輪工具呼叫 / 智能體任務中的淨改進幅度。
* **確認成功率**：任務完成後使用者確認成功（讚）的比例。
* **讚踩比**：使用者對回答投「讚」與「踩」的比例，反映主觀滿意度。
* **可控性**：模型遵循指令並保持穩定輸出的能力。
* **Bash 恢復率**：在 Bash / 命令列操作出錯後能自我恢復的比例。
* **工具幻覺率**：模型虛構或呼叫不存在工具的比例，越低越好。
* **會話數**：樣本量參考；樣本較少時，排名通常更容易波動。

## 選擇模型時再確認三件事

1. Provider 是否實際提供這個模型，以及你的地區和帳戶是否可用；
2. API 價格、速率限制和上下文是否適合你的任務；
3. 用 3～5 個真實任務做小規模測試，不要只看總榜名次。

## 資資來源

資資來自 [Arena AI 官方 Agent 智能體榜單](https://arena.ai/leaderboard/agent)，由 GitHub Actions 每天更新。模型價格、授權條款和能力請以模型服務商官方資訊為準。
