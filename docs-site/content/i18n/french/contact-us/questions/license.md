...
le code source ou pouvez l'obtenir si vous le souhaitez, que vous pouvez
...
développeurs de logiciels libres sont encouragés et motivés par la
coopération qui en résulte. Cependant, dans le cas des logiciels utilisés
sur des serveurs réseau, ce résultat peut ne pas se produire. La Licence
Générale du Public GNU permet de créer une version modifiée et de laisser
...
de ce serveur. Par conséquent, l'utilisation publique d'une version
...
La propagation inclut la copie, la distribution (avec ou sans
...
l'œuvre (sauf dans la mesure où des garanties sont fournies), que les
licenciés peuvent transmettre l'œuvre sous cette Licence, et comment
consulter une copie de cette Licence. Si l'interface présente une liste
...
Une « interface standard » désigne une interface qui est soit une norme officielle définie par un organisme de normalisation reconnu, soit, dans le cas des interfaces spécifiées pour un langage de programmation particulier, une interface largement utilisée parmi les développeurs travaillant dans ce langage.

Les « bibliothèques système » d'une œuvre exécutable incluent tout élément, à l'exception de l'œuvre dans son ensemble, qui (a) est inclus dans la forme normale d'emballage d'un composant majeur, mais qui ne fait pas partie de ce composant majeur, et (b) sert uniquement à permettre l'utilisation de l'œuvre avec ce composant majeur, ou à implémenter une interface standard pour laquelle une implémentation est disponible au public sous forme de code source. Un « composant majeur », dans ce contexte, désigne un composant essentiel majeur (noyau, système de fenêtres, etc.) du système d'exploitation spécifique (le cas échéant) sur lequel l'œuvre exécutable s'exécute, ou un compilateur utilisé pour produire l'œuvre, ou un interpréteur de code objet utilisé pour l'exécuter.

Le « code source correspondant » d'une œuvre sous forme de code objet désigne tout le code source nécessaire pour générer, installer et (pour une œuvre exécutable) exécuter le code objet et modifier l'œuvre, y compris les scripts pour contrôler ces activités. Toutefois, il n'inclut pas les bibliothèques système de l'œuvre, ni les outils à usage général ou les programmes libres généralement disponibles qui sont utilisés sans modification pour effectuer ces activités mais qui ne font pas partie de l'œuvre. Par exemple, le code source correspondant inclut les fichiers de définition d'interface associés aux fichiers source de l'œuvre, et le code source des bibliothèques partagées et des sous-programmes liés dynamiquement que l'œuvre est spécifiquement conçue pour requérir, tels que par une communication de données intime ou un flux de contrôle entre ces sous-programmes et d'autres parties de l'œuvre.

Le code source correspondant n'a pas besoin d'inclure tout ce que les utilisateurs peuvent régénérer automatiquement à partir d'autres parties du code source correspondant.
...
Les « informations d'installation » pour un produit utilisateur désignent toutes les méthodes, procédures, clés d'autorisation ou autres informations requises pour installer et exécuter des versions modifiées d'une œuvre couverte dans ce produit utilisateur à partir d'une version modifiée de sa source correspondante. Ces informations doivent suffire à garantir que le fonctionnement continu du code objet modifié n'est en aucun cas empêché ou entravé uniquement en raison de la modification effectuée.

Si vous transmettez une œuvre en code objet au titre de la présente section, dans, avec ou spécifiquement pour l'utilisation dans un produit utilisateur, et que la transmission s'effectue dans le cadre d'une transaction au cours de laquelle le droit de possession et d'utilisation du produit utilisateur est transféré au bénéficiaire à perpétuité ou pour une durée déterminée (quelle que soit la qualification de la transaction), la source correspondante transmise au titre de la présente section doit être accompagnée des informations d'installation. Toutefois, cette exigence ne s'applique pas si ni vous ni un tiers ne conservez la capacité d'installer du code objet modifié sur le produit utilisateur (par exemple, si l'œuvre a été installée en ROM).

L'obligation de fournir des informations d'installation n'inclut pas l'obligation de continuer à fournir un service de support, une garantie ou des mises à jour pour une œuvre qui a été modifiée ou installée par le bénéficiaire, ou pour le produit utilisateur dans lequel elle a été modifiée ou installée. L'accès à un réseau peut être refusé lorsque la modification elle-même affecte de manière substantielle et défavorable le fonctionnement du réseau ou viole les règles et protocoles de communication à travers le réseau.
...
Lorsque vous transmettez une copie d'une œuvre couverte, vous pouvez, à votre option, supprimer toute permission supplémentaire de cette copie, ou de toute partie de celle-ci. (Les permissions supplémentaires peuvent être rédigées pour exiger leur propre suppression dans certains cas lorsque vous modifiez l'œuvre.) Vous pouvez accorder des permissions supplémentaires à du matériel que vous ajoutez à une œuvre couverte, pour lequel vous avez ou pouvez donner une autorisation de droit d'auteur appropriée.
...
Si vous ajoutez des termes à une œuvre couverte conformément à la présente section, vous devez placer, dans les fichiers source pertinents, une déclaration des termes supplémentaires applicables à ces fichiers, ou un avis indiquant où trouver les termes applicables.
...
Toutefois, si vous cessez toute violation de la présente Licence, votre licence accordée par un titulaire de droits d’auteur particulier est rétablie (a) provisoirement, sauf si le titulaire de droits d’auteur met fin explicitement et définitivement à votre licence, et (b) définitivement, si le titulaire de droits d’auteur ne vous notifie pas la violation par un moyen raisonnable avant 60 jours après le cessez-le-yez.

De plus, votre licence accordée par un titulaire de droits d’auteur particulier est rétablie définitivement si ce titulaire vous notifie la violation par un moyen raisonnable, qu’il s’agit de la première fois que vous recevez une notification de violation de la présente Licence (pour toute œuvre) de la part de ce titulaire, et que vous remédiez à la violation dans les 30 jours suivant la réception de la notification.
...
Une « transaction d’entité » est une transaction transférant le contrôle d’une organisation, ou l’essentiel de ses actifs, ou divisant une organisation, ou fusionnant des organisations. Si la propagation d’une œuvre couverte résulte d’une transaction d’entité, chaque partie à cette transaction qui reçoit une copie de l’œuvre reçoit également toutes les licences sur l’œuvre dont le prédécesseur de la partie dans l’intérêt disposait ou pouvait accorder en vertu du paragraphe précédent, ainsi qu’un droit de possession du Source Correspondant de l’œuvre par le prédécesseur dans l’intérêt, si ce dernier le possède ou peut l’obtenir par des efforts raisonnables.
...
Les « revendications de brevets essentielles » d’un contributeur désignent l’ensemble des revendications de brevets dont le contributeur est propriétaire ou qu’il contrôle, qu’elles aient déjà été acquises ou qu’elles le soient à l’avenir, qui seraient enfreintes par une quelconque manière de fabriquer, utiliser ou vendre sa version de contributeur, permise par la présente Licence, mais excluent les revendications qui ne seraient enfreintes que par la conséquence d’une modification ultérieure de la version du contributeur. Aux fins de cette définition, le terme « contrôle » inclut le droit d’accorder des sous-licences de brevets d’une manière conforme aux exigences de la présente Licence.

Chaque contributeur vous accorde une licence de brevet non exclusive, mondiale et sans redevance, au titre des revendications de brevets essentielles du contributeur, pour fabriquer, utiliser, vendre, offrir à la vente, importer et, plus généralement, exécuter, modifier et propager le contenu de sa version de contributeur.

Dans les trois paragraphes suivants, une « licence de brevet » désigne tout accord ou engagement exprès, quel que soit son intitulé, de ne pas faire valoir un brevet (tel qu’une permission exprès de mettre en œuvre un brevet ou un engagement de ne pas intenter d’action en contrefaçon de brevet). « Accorder » une telle licence de brevet à une partie signifie conclure un tel accord ou engagement de ne pas faire valoir un brevet contre cette partie.

Si vous transmettez une œuvre couverte en s’appuyant sciemment sur une licence de brevet, et que la Source Correspondante de l’œuvre n’est pas disponible pour quiconque à copier, gratuitement et aux termes de la présente Licence, via un serveur réseau publiquement accessible ou tout autre moyen facilement accessible, vous devez soit (1) rendre la Source Correspondante disponible de cette manière, soit (2) vous priver du bénéfice de la licence de brevet pour cette œuvre particulière, soit (3) prévoir, d’une manière conforme aux exigences de la présente Licence, d’étendre la licence de brevet aux destinataires en aval. « S’appuyer sciemment » signifie que vous avez la connaissance effective que, sans la licence de brevet, votre transmission de l’œuvre couverte dans un pays, ou l’utilisation de l’œuvre couverte par votre destinataire dans un pays, enfreindrait un ou plusieurs brevets identifiables dans ce pays que vous avez des raisons de croire valides.

Si, en vertu ou en relation avec une seule transaction ou un seul arrangement, vous transmettez, ou propagez en provoquant la transmission de, une œuvre couverte, et accordez une licence de brevet à certaines des parties recevant l’œuvre couverte, les autorisant à utiliser, propager, modifier ou transmettre une copie spécifique de l’œuvre couverte, alors la licence de brevet que vous accordez est automatiquement étendue à tous les destinataires de l’œuvre couverte et des œuvres qui en sont dérivées.

Une licence de brevet est « discriminatoire » si elle n’inclut pas dans le champ de sa couverture, interdit l’exercice de, ou est subordonnée à la non-exercice de, l’un ou plusieurs des droits spécifiquement accordés par la présente Licence. Vous ne pouvez pas transmettre une œuvre couverte si vous êtes partie à un arrangement avec un tiers qui exerce une activité de distribution de logiciels, en vertu duquel vous versez un paiement au tiers basé sur l’étendue de votre activité de transmission de l’œuvre, et en vertu duquel le tiers accorde, à l’une des parties qui recevraient l’œuvre couverte de votre part, une licence de brevet discriminatoire (a) en relation avec les copies de l’œuvre couverte transmises par vous (ou les copies faites à partir de ces copies), ou (b) principalement pour et en relation avec des produits ou des compilations spécifiques contenant l’œuvre couverte, à moins que vous n’ayez conclu cet arrangement, ou que cette licence de brevet n’ait été accordée, avant le 28 mars 2007.
...
utilisateurs interagissant avec elle à distance via un réseau informatique (si votre version
prend en charge une telle interaction) la possibilité de recevoir le Source Correspondant
de votre version en fournissant un accès au Source Correspondant
depuis un serveur réseau gratuitement, par un moyen standard ou habituel
de faciliter la copie de logiciels. Ce Source Correspondant
doit inclure le Source Correspondant de tout travail couvert par la version 3
de la Licence Publique Générale GNU qui est incorporé conformément au
paragraphe suivant.

Nonobstant toute autre disposition de la présente Licence, vous avez
la permission de lier ou de combiner tout travail couvert avec un travail sous licence
version 3 de la Licence Publique Générale GNU en un seul
travail combiné, et de transmettre le travail résultant. Les termes de cette
Licence continueront de s'appliquer à la partie qui est le travail couvert,
...
seront similaires en esprit à la version actuelle, mais peuvent différer dans le détail pour
traiter de nouveaux problèmes ou préoccupations.

Chaque version est attribuée un numéro de version distinctif. Si le
Programme spécifie qu'une version numérotée spécifique de la Licence Publique Générale
GNU Affero « ou toute version ultérieure » s'applique à lui, vous avez
l'option de suivre les termes et conditions de cette version numérotée
ou de toute version ultérieure publiée par la Free Software
Foundation. Si le Programme ne spécifie pas de numéro de version de la
Licence Publique Générale GNU Affero, vous pouvez choisir toute version publiée
par la Free Software Foundation.

Si le Programme spécifie qu'un mandataire peut décider quelles
versions futures de la Licence Publique Générale GNU Affero peuvent être utilisées, la déclaration
publique d'acceptation d'une version par ce mandataire vous autorise
permanemment à choisir cette version pour le Programme.

Les versions ultérieures de la licence peuvent vous accorder des permissions supplémentaires ou différentes.
...
DE DROITS D'AUTEUR ET/OU D'AUTRES PARTIES FOURNISSENT LE PROGRAMME « EN L'ÉTAT » SANS AUCUNE
...
PARTICULIER. L'ENTIER RISQUE CONCERNANT LA QUALITÉ ET LES PERFORMANCES DU PROGRAMME
...
LE PROGRAMME CONFORMÉMENT À CE QUI PRÉCÈDE, NE SAURAIT ÊTRE TENU RESPONSABLE À VOTRE ÉGARD DE DOMMAGES, Y COMPRIS TOUT
DOMMAGE GÉNÉRAL, SPÉCIAL, ACCIDENTEL OU INDIRECT DÉCOULANT DE L'UTILISATION
OU DE L'IMPOSSIBILITÉ D'UTILISER LE PROGRAMME (Y COMPRIS, SANS S'Y LIMITER, LA PERTE
DE DONNÉES OU LE RENDU INEXACT DES DONNÉES OU LES PERTES SUBIES PAR VOUS OU DES TIERS
OU L'ÉCHEC DU PROGRAMME À FONCTIONNER AVEC D'AUTRES PROGRAMMES),
...
les juridictions compétentes appliqueront la loi locale qui s'approche le plus
d'une renonciation absolue de toute responsabilité civile en lien avec le
...
logiciel libre que chacun peut redistribuer et modifier selon ces termes.

Pour ce faire, joignez les avis suivants au programme. Il est le plus sûr
de les joindre au début de chaque fichier source pour exprimer le plus efficacement
...
Si votre logiciel peut interagir à distance avec les utilisateurs via un réseau informatique, vous devez également vous assurer qu'il offre un moyen pour les utilisateurs d'obtenir son code source. Par exemple, si votre programme est une application web, son interface pourrait afficher un lien « Source » qui mène les utilisateurs vers une archive du code. Il existe de nombreuses façons d'offrir le code source, et différentes solutions seront plus adaptées à différents programmes ; voir la section 13 pour les exigences spécifiques.

Vous devriez également faire signer à votre employeur (si vous travaillez comme programmeur) ou à votre établissement d'enseignement, le cas échéant, une « décharge de droits d'auteur » pour le programme, si nécessaire. Pour plus d'informations à ce sujet, et sur la manière d'appliquer et de respecter la GNU AGPL, voir <https://www.gnu.org/licenses/>.
