...
На основе S3-совместимого хранилища можно реализовать синхронизацию данных между устройствами по схеме: `Компьютер A`  → (резервное копирование) →  `S3-хранилище`  → (восстановление) →  `Компьютер B`.
