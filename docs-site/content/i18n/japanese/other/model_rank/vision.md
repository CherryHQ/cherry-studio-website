...
このページでは、[Arena AI](https://arena.ai/leaderboard/vision) ランキング上位30位の1日ごとのスナップショットを表示し、直近のモデルの性能を素早く把握できるようにしています。完全なランキング、フィルターオプション、および最新の更新状況については、Arena AI の公式サイトをご確認ください。

本ランキングでは、マルチモーダルモデルの画像理解タスクにおける性能を評価しています。

> **データ更新時刻**: 2026-09-19 12:14:58 UTC / 2026-09-19 20:14:58 CST (北京時間)

{% hint style="info" %}
ランキングは特定の評価基準とユーザーの投票傾向を反映しており、必ずしもあなたのタスクにおいてモデルが優れていることを意味しません。モデルを選択する際は、価格、速度、コンテキスト、ツール呼び出し、プライバシー、地域利用可能性なども考慮してください。
{% endhint %}
...
* **スコア**：相対的な評価点で、同一時点のランキング内のモデル比較に適しています。
* **投票数 / セッション数**：サンプルサイズの参考値です。サンプル数が少ない場合、ランキングは変動しやすくなります。
* **価格 $/100万Token**：入力 / 出力あたり100万Tokenの参考価格です。
* **コンテキスト**：モデルがサポートする最大コンテキスト長です。

## モデル選択時に確認すべき3つのこと

1. Provider が実際にそのモデルを提供しているか、およびあなたの地域とアカウントで利用可能か；
2. API の価格、レート制限、コンテキストがあなたのタスクに適しているか；
3. 総ランキングの順位だけでなく、3〜5つの実際のタスクで小規模なテストを行うこと。

## データソース

データは [Arena AI 公式 ビジュアルランキング](https://arena.ai/leaderboard/vision) に由来し、GitHub Actions によって毎日更新されます。モデルの価格、ライセンス、機能については、モデルプロバイダーの公式情報をご確認ください。
